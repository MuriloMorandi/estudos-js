...
import { tasks, users } from "@estudos/main-db";
...
export class TaskService implements IBaseService<Task, Task["id"]> {
	...
	async getAll(data: IPagination): Promise<{ items: Task[]; total: number }> {
		const colunaOrdenacao = <AnyColumn>tasks[<keyof typeof tasks>data.sortBy];
		...
		const [total] = await this._database.select({ count: count() }).from(tasks).execute();
	...
	async create(data: typeof tasks.$inferInsert): Promise<Task> {
		const [ret] = await this._database.insert(tasks).values(data).returning().execute();
	...
	async delete(id: Task["id"]): Promise<void> {
		await this._database.delete(tasks).where(eq(tasks.id, id)).execute();
	}
}
