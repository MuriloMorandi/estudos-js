...
describe("Schema de paginação", () => {
	...
	it("deve falhar quando perPage não for um inteiro positivo", () => {
		...
		if (!result.success && result.error && result.error.errors[0]) {
			expect(result.error.errors[0].message).toBe("O número de itens por página deve ser maior que 0.");
		}
	});
	...
	it("deve falhar quando pagina não for um inteiro positivo", () => {
		...
		expect(result.success).toBe(false);
		if (!result.success && result.error && result.error.errors[0]) {
			expect(result.error.errors[0].message).toBe("O número da página deve ser maior que 0.");
		}
	});
